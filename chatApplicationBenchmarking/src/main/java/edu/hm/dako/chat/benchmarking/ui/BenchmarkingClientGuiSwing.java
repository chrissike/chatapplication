...
import edu.hm.dako.chat.benchmarking.data.UserInterfaceInputParameters.MeasurementType;
 ...
 *
 */
public class BenchmarkingClientGuiSwing extends JPanel implements BenchmarkingClientUserInterface, ActionListener {
	...
	private void initComponents() {
		 ...
		 */
		String[] optionStrings = { IMPL_TCP_ADVANCED, IMPL_TCP_SIMPLE, IMPL_UDP_ADVANCED };
	...
	public JComponent buildPanel() {
		...
		panelBenchmarkingClientGui.add(new JLabel("Vom Server gesendete Event-Nachrichten"), cc.xy(5, 26));
		panelBenchmarkingClientGui.add(textFieldSentEventMessages, cc.xy(7, 26));
		textFieldSentEventMessages.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("Vom Server empfangene Confirm-Nachrichten"), cc.xy(1, 28));
		...
		panelBenchmarkingClientGui.add(createSeparator("Statistische Auswertung"), cc.xyw(1, 37, 7));

		panelBenchmarkingClientGui.add(new JLabel("Mittlere RTT (Arithm. Mittel) in ms"), cc.xy(1, 39));
		panelBenchmarkingClientGui.add(textFieldAvgRTT, cc.xy(3, 39));
		textFieldAvgRTT.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("50%-Percentile (Median) in ms"), cc.xy(5, 39));
		panelBenchmarkingClientGui.add(textField50Percentile, cc.xy(7, 39));
		textField50Percentile.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("Maximale RTT in ms"), cc.xy(1, 41));
		...
		panelBenchmarkingClientGui.add(new JLabel("Interquartilsabstand in ms"), cc.xy(5, 45));
		panelBenchmarkingClientGui.add(textFieldInterquartilRange, cc.xy(7, 45));
		textFieldAvgCpuUsage.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("25%-Percentile in ms"), cc.xy(1, 47));
		panelBenchmarkingClientGui.add(textField25Percentile, cc.xy(3, 47));
		textField25Percentile.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("Mittlere Serverzeit in ms"), cc.xy(5, 47));
		panelBenchmarkingClientGui.add(textFieldAvgServerTime, cc.xy(7, 47));
		textFieldAvgServerTime.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("75%-Percentile in ms"), cc.xy(1, 49));
		panelBenchmarkingClientGui.add(textField75Percentile, cc.xy(3, 49));
		textField75Percentile.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("Maximale Heap-Belegung in MiB"), cc.xy(5, 49));
		panelBenchmarkingClientGui.add(textFieldMaxHeapUsage, cc.xy(7, 49));
		textFieldMaxHeapUsage.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("90%-Percentile in ms"), cc.xy(1, 51));
		panelBenchmarkingClientGui.add(textField90Percentile, cc.xy(3, 51));
		textField90Percentile.setEditable(false);

		panelBenchmarkingClientGui.add(new JLabel("Durchschnittliche CPU-Auslastung in %"), cc.xy(5, 51));
	...
	/**
	 * Aktion bei Betaetigung des "Start"-Buttons ausfuehren Eingabefelder
	 * werden validiert.
	 *
	 * @param e
	 *            Ereignis
	...
	public synchronized void showResultData(UserInterfaceResultData data) {
		...
		formatter = new Formatter();
		textField10Percentile.setText(formatter.format("%.2f", (new Double(data.getPercentile10()))).toString());
		formatter = new Formatter();
		textField25Percentile.setText(formatter.format("%.2f", (new Double(data.getPercentile25()))).toString());
		formatter = new Formatter();
		textField50Percentile.setText(formatter.format("%.2f", (new Double(data.getPercentile50()))).toString());
		formatter = new Formatter();
		textField75Percentile.setText(formatter.format("%.2f", (new Double(data.getPercentile75()))).toString());
		formatter = new Formatter();
		textField90Percentile.setText(formatter.format("%.2f", (new Double(data.getPercentile90()))).toString());
		formatter = new Formatter();
